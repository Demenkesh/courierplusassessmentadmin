    ...
    <div class="relative flex flex-col justify-center min-h-screen overflow-hidden">
        <div
            ...
            <div class="text-center p-6 rounded-t">
                <a href=""><img src="https://flowbite.com/docs/images/logo.svg" alt="" class="w-14 h-14 mx-auto mb-2"></a>
                <h3 class="font-semibold text-black text-xl mb-1">Let's Get Started with {{ getTenantName() }}</h3>
                <p class="text-xs text-slate-400">Sign in to continue to {{ getTenantName() }}.</p>
            </div>

            <form action="{{ url('tenant-auth/register') }}" method="POST" class="p-6">
                @csrf
                <div>
                    <label for="User_Name" class="font-medium text-sm text-slate-600 dark:text-slate-400">Name</label>
                ...
                <div class="block mt-3">
                    <label class="custom-label block dark:text-slate-300">
                        <div
                            class="bg-white dark:bg-slate-700  border border-slate-700 dark:border-slate-600 rounded w-4 h-4  inline-block leading-4 text-center -mb-[3px]">
                            <input type="checkbox" class="hidden">
                            <i class="fas fa-check hidden text-xs text-slate-700 dark:text-slate-200"></i>
                        </div>
                        By registering you agree to the {{ getTenantName() }} Terms of Use
                    </label>
                </div>
                <div class="mt-4">
                    <button
            ...
            <p class="mb-5 text-sm font-medium text-center text-slate-500"> Already have an account ? <a
                    href="{{ url('tenant-auth/login') }}" class="font-medium text-brand-500 hover:underline">Log in</a>
            </p>
        </div>
    </div>
@endsection
